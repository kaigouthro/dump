...
class TaskStorage:
    ...
    Usage:
        ...
        - Use the save_task() method to save a new task with its details such as task name, description, completeness, parent task ID, etc.
    ...
    def check_task_exists(self, task, description):
        """
        ...
        cursor.execute(
            "SELECT EXISTS(SELECT 1 FROM tasks WHERE task = ? AND description = ?)",
            (task, description),
        )
    ...
    def update_task(
        ...
        if update_fields:
            with self.conn:
                self.conn.execute(
                    "UPDATE tasks SET {} WHERE id = ?".format(", ".join(update_fields)),
                    params + [task_id],
                )
